  ...
  return (
    <div className="min-h-screen bg-[#f8f8f6]">
      ...
      <div className="max-w-sm mx-auto px-4 py-6">
        <div className="page-header mb-5"><h1>Vote on dates</h1><p>Tell us which dates work for you</p></div>
            ...
            return (
              <div key={opt.id} className={`card ${opt.is_locked?'border-green-200 bg-green-50':''}`}>
                <div className="flex items-start justify-between mb-3">
                  <div><p className="font-medium text-sm text-gray-900">{dateStr}</p><p className="text-xs text-gray-500">{opt.start_time} – {opt.end_time} · {event.location}</p></div>
                  {opt.is_locked?<span className="badge-teal">Locked ✓</span>:<span className="badge-amber">Open</span>}
                </div>
                <div className="mb-3">
                  <div className="flex justify-between text-xs text-gray-500 mb-1"><span>✅ {c.can_go} can go</span><span>❌ {c.cant_go} can't go</span></div>
                  <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-green-500 rounded-full transition-all" style={{width:`${pct}%`}} /></div>
                </div>
                {!opt.is_locked && event?.status==='voting' && (
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={()=>castVote(opt.id,'can_go')} disabled={saving===opt.id}
                      className={`py-2.5 rounded-xl text-sm font-medium border transition-all active:scale-95 ${myVote==='can_go'?'bg-green-500 text-white border-green-500':'bg-white text-gray-700 border-gray-200 hover:bg-green-50'}`}>
                      ✅ I can go
                    </button>
                    <button onClick={()=>castVote(opt.id,'cant_go')} disabled={saving===opt.id}
                      className={`py-2.5 rounded-xl text-sm font-medium border transition-all active:scale-95 ${myVote==='cant_go'?'bg-red-500 text-white border-red-500':'bg-white text-gray-700 border-gray-200 hover:bg-red-50'}`}>
                      ❌ Can't go
                    </button>
                  </div>
                )}
                {showReason[opt.id] && (
                  <div className="mt-3 space-y-2">
                    <label className="label text-red-600">Why can't you make it? (required)</label>
                    ...
                    <button onClick={()=>submitReason(opt.id)} disabled={!reasons[opt.id]?.trim()||saving===opt.id}
