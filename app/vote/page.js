...
export default function VoteIndex() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setProfile(prof)
      const { data: evs } = await supabase.from('events').select('*, date_options(*)').eq('status', 'voting').order('created_at', { ascending: false })
      const active = evs || []
      if (active.length === 1) { router.push('/vote/' + active[0].id); return }
      setEvents(active)
      setLoading(false)
    }
    load()
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F8]">
      ...
      <div className="max-w-sm mx-auto px-4 py-6">
        <div className="page-header mb-5"><h1>Vote on dates</h1><p>Active voting sessions</p></div>
        {events.length === 0 && (
          <div className="card text-center py-12 fade-in">
            ...
            <p className="text-xs text-gray-400 mt-1">The manager has not opened voting yet</p>
          </div>
        )}
            ...
            <div key={ev.id} className="card fade-in border-l-4" style={{borderLeftColor:'var(--gold)'}}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{ev.date_options?.length || 0} date options open</p>
                </div>
                <span className="badge-gold">Open</span>
              </div>
              <button onClick={() => router.push('/vote/' + ev.id)} className="btn-primary text-sm">Vote now →</button>
            </div>
          ))}
        </div>
      </div>
